...
class Answer2 extends Controller
{
    // 查找表
    private $searchList, $count;
    ...
    public function calTotalWay(int $n): int
    {
        ...
        //假設查找表不存在這個數字
        if (!isset($this->searchList[$n])) {
            $this->searchList[$n] = $this->calTotalWay($n - 1) + $this->calTotalWay($n - 2);
        }
        return $this->searchList[$n];
    }
